...
const API = 'http://localhost:4000/api'; // using 4001 or 4000? Let's check environment or try 4001 as logged. Actually 4001.
...
async function runE2E() {
  ...
  for (let loop = 1; loop <= 2; loop++) {
    ...
    if (!dlpiId) {
      ...
      // In real life, Patwari creates it, but we'll assume there's one. Let's just create one manually in Mongo for the test.
      const connectDB = require('../../backend/api-gateway/src/config/db');
      const DLPI = require('../../backend/api-gateway/src/models/DLPI');
      await connectDB();
    ...
    const connectDB = require('../../backend/api-gateway/src/config/db');
    const Mutation = require('../../backend/api-gateway/src/models/Mutation');
    ...
    const mutation = await Mutation.findOne({ dlpiId: dlpiId, status: 'PENDING_CI_APPROVAL' }).sort({ createdAt: -1 });
    
    if (mutation) {
      console.log(`✅ ZERO-CLICK MUTATION VERIFIED! Case ID: ${mutation.caseId}`);
      console.log(`   New Owner: ${mutation.newOwnerDetails.name} (${mutation.newOwnerDetails.aadhaarNumber})`);
    } else {
...
// Add mock env var so authenticate works
process.env.AADHAAR_MOCK = 'true';
// Provide MONGODB_URI for direct DB checks
