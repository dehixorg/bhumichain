...
CLI_CONTAINER=$(docker ps --format '{{.Names}}' | grep 'cli' | head -n 1)
if [ -z "$CLI_CONTAINER" ]; then
  echo "Error: CLI container not found. Is the Fabric network running?"
  exit 1
fi
echo "Using CLI container: $CLI_CONTAINER"

echo "  [1/5] Packaging..."
docker exec "$CLI_CONTAINER" peer lifecycle chaincode package "/tmp/${NAME}.tar.gz" \
  --path "$CC_SRC_PATH" \
  --lang golang \
  --label "${NAME}_${VERSION}"

echo "  [2/5] Installing on peer0..."
docker exec "$CLI_CONTAINER" peer lifecycle chaincode install "/tmp/${NAME}.tar.gz" || true
...
if [ "$PEER1_RUNNING" = "true" ]; then
  ...
  docker exec \
    ...
    "$CLI_CONTAINER" peer lifecycle chaincode install "/tmp/${NAME}.tar.gz" || true
fi

echo "  [3/5] Getting package ID..."
CC_PACKAGE_ID=$(docker exec "$CLI_CONTAINER" peer lifecycle chaincode queryinstalled \
  --output json | \
...
docker exec "$CLI_CONTAINER" peer lifecycle chaincode approveformyorg \
  --channelID "$CHANNEL" \
  --name "$NAME" \
  --version "$VERSION" \
...
docker exec "$CLI_CONTAINER" peer lifecycle chaincode commit \
  --channelID "$CHANNEL" \
  --name "$NAME" \
  --version "$VERSION" \
  --sequence "$SEQUENCE" \
