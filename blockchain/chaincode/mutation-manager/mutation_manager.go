...
func (c *MutationManagerContract) InitiateMutation(
	...
	if requiresUttaradhikarOrigin[mutationType] {
		if initiatedBy != "UTTARADHIKAR_ENGINE" {
			return "", fmt.Errorf(
				"ANTI_CORRUPTION: INHERITANCE and SUCCESSION_CLAIM mutations cannot be manually filed by officers. "+
					"They must be auto-generated by the Uttaradhikar Engine after all heirs consent. "+
					"Officer %s (%s) attempted manual %s mutation on DLPI %s. "+
					"This attempt is permanently recorded.",
				officerName, officerHash, mutationType, dlpiId,
			)
		}
		if successionCaseId == "" {
			return "", fmt.Errorf("%s mutation requires a valid successionCaseId from Uttaradhikar Engine", mutationType)
		}
	}
	...
	if mutationType == "COURT_ORDER" || mutationType == "INHERITANCE" || mutationType == "SUCCESSION_CLAIM" {
		initialStatus = MutStatusPendingExecution
	} else if len(currentOwnerRefs) == 0 {
		initialStatus = MutStatusPendingExecution
	}
...
func (c *MutationManagerContract) ExecuteMutation(
	...
	if mutation.MutationType == "SUCCESSION_CLAIM" {
		// Bypasses further SLA / officer accountability checks since it is an automated Smart Contract Will execution
	}
